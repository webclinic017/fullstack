module.exports = {
    tobeMaster0:{
        zh:"成为明星交易员",
        en:"Become TigerWit's Master",
        vi:"Trở thành Bậc Thầy của TigerWit",
        "zh-Hant":"成為明星交易員",
        id:"Jadilah seorang Master di TigerWit"
    },
    tobeMaster1:{
        zh:"享受所有跟随用户20%的盈利",
        en:"Get 20% profit share",
        vi:"Nhận 20% lợi nhuận",
        "zh-Hant":"享受所有跟隨用戶20%的盈利",
        id:"Dapatkan bagi hasil hingga 20%"
    },
    tobeMaster2:{
        zh:"如何登上TigerWit高手榜",
        en:"How to get on the TigerWit Masters' List",
        vi:"Làm thế nào để có được danh sách bậc thầy của TigerWit",
        "zh-Hant":"如何登上TigerWit高手榜",
        id:"Bagaimana cara menjadi Master di TigerWit?"
    },
    tobeMaster3:{
        zh:"TigerWit对交易员榜单按照其交易表现进行排名，表现越优秀排名越居前端。",
        en:"TigerWit ranks according to master's trading performance, the better the performance, the better the ranking is.",
        vi:"TigerWit xếp hạng theo hiệu suất giao dịch của các bậc thầy, hiệu suất càng tốt thì xếp hạng càng tốt.",
        "zh-Hant":"TigerWit對交易員榜單按照其交易表現進行排名，表現越優秀排名越居前端。",
        id:"Peringkat Master di TigerWit ditentukan berdasarkan hasil trading, peringkat semakin tinggi menandakan hasil yang diperoleh juga semakin baik."
    },
    tobeMaster4_1:{
        zh:"初步-完善信息：",
        en:"First Step: ",
        vi:"Bước đầu tiên",
        "zh-Hant":"初步-完善信息：",
        id:"Langkah pertama: "
    },
    tobeMaster4_2:{
        zh:"开通TigerWit真实账户，并完成实名认证。",
        en:"Open a TigerWit live account, and complete the verification process.",
        vi:"Mở tài khoản trực tiếp TigerWit và hoàn tất quá trình xác minh.",
        "zh-Hant":"開通TigerWit真實賬戶，並完成實名認證。",
        id:"Buka Akun Live di TigerWit dan lengkapi proses verifikasi."
    },
    tobeMaster5_1:{
        zh:"进阶-业绩指标：",
        en:"Second Step: ",
        vi:"Bướ thứ 2: ",
        "zh-Hant":"進階-業績指標：",
        id:"Langkah kedua: "
    },
    tobeMaster5_2:{
        zh:"TigerWit交易员榜单随着交易表现变动，进阶为入围交易高手的要求：",
        en:"TigerWit’s master list changes with the trading performance, demands of become a finalist are as follow: ",
        vi:"Danh sách bậc thầy tổng thể của TigerWit thay đổi với hiệu suất giao dịch, nhu cầu trở thành nhà giao dịch cuối cùng như sau: ",
        "zh-Hant":"TigerWit交易員榜單隨著交易表現變動，進階為入圍交易高手的要求：",
        id:"Untuk bisa muncul di daftar Master, anda harus memenuhi kriteria sebagai berikut: "
    },
    tobeMaster6:{
        zh:"在老虎外汇交易平台入金并连续自主交易20个以上工作日；",
        en:"Make minimum initial deposit and self-trade for at least 20 trading days;",
        vi:"Gửi tối thiểu tiền gửi ban đầu và tự giao dịch trong ít nhất 20 ngày giao dịch;",
        "zh-Hant":"在老虎外匯交易平臺入金並連續自主交易20個以上工作日；",
        id:"Mengisi dana dan melakukan transaksi selama minimal 20 hari kerja;"
    },
    tobeMaster7:{
        zh:"近30日收益率大于20%；",
        en:"Your overall profit must be 20% or more;",
        vi:"Tổng lợi nhuận của bạn phải là 20% trở lên;",
        "zh-Hant":"近30日收益率大於20%；",
        id:"Total keuntungan anda lebih dari atau sama dengan 20%;"
    },
    tobeMaster8:{
        zh:"账户余额不得低于500美金。",
        en:"Your overall account equity is no less than USD500.",
        vi:"Tổng equity tài khoản của bạn không ít hơn 500 đô la.",
        "zh-Hant":"賬戶余額不得低於500美金。",
        id:"Total saldo anda tidak boleh kurang dari 500 $."
    },
    tobeMaster9_1:{
        zh:"终极-登上高手榜：",
        en:"At Last: ",
        vi:"Cuối cùng:",
        "zh-Hant":"終極-登上高手榜：",
        id:"Terakhir: "
    },
    tobeMaster9_2:{
        zh:"TigerWit会在入围交易者中综合评定上榜高手，影响上榜的决定性因素有：",
        en:"TigerWit will comprehensively assess the finalists,in order to decide who can get on the masters' list. The decisive factors are as follow: ",
        vi:"TigerWit sẽ đánh giá toàn diện những người vào danh sách cuối cùng, để quyết định ai có thể vào danh sách của các bậc thầy. Các yếu tố quyết định như sau: ",
        "zh-Hant":"TigerWit會在入圍交易者中綜合評定上榜高手，影響上榜的決定性因素有：",
        id:"TigerWit selalu melakukan evaluasi kepada setiap Master, dan berhak memutuskan siapa saja yang berhak menyandang status tersebut. Berikut adalah faktor penentu dalam pengambilan keputusan:"
    },
    tobeMaster10:{
        zh:"总收益率：交易员交易收益比率。总收益率=平仓盈亏总和/入金总和*100%；",
        en:"Total Profit Rate: The trader's trading income ratio.",
        vi:"Tổng tỷ suất lợi nhuận: Tỷ lệ thu nhập giao dịch của nhà giao dịch.",
        "zh-Hant":"總收益率：交易員交易收益比率。總收益率=平倉盈虧總和/入金總和*100%；",
        id:"Total keuntungan rata-rata : Rasio keuntungan dari hasil trading."
    },
    tobeMaster11:{
        zh:"交易订单量：交易员交易订单的数量。考量交易活跃度，交易订单量与交易活跃度成正比；",
        en:"Order quantity：The number of trading orders.",
        vi:"Số lượng đặt hàng: Số lượng lệnh giao dịch.",
        "zh-Hant":"交易訂單量：交易員交易訂單的數量。考量交易活躍度，交易訂單量與交易活躍度成正比；",
        id:"Total Transaksi : jumlah transaksi yang dieksekusi."
    },
    tobeMaster12:{
        zh:"胜率：即交易准确率。胜率=平仓盈利订单/总平仓订单*100%。",
        en:"Accuracy：The proportion of profitable orders.",
        vi:"Độ chính xác: Tỷ lệ các lệnh có lãi.",
        "zh-Hant":"勝率：即交易準確率。勝率=平倉盈利訂單/總平倉訂單*100%。",
        id:"Akurasi : Persentase transaksi yang menang dan kalah."
    },
    tobeMaster13:{
        zh:"交易高手权益和义务",
        en:"Master's Rights and Obligations",
        vi:"Quyền và nghĩa vụ của Bậc Thầy",
        "zh-Hant":"交易高手權益和義務",
        id:"Hak dan Kewajiban seorang Master"
    },
    tobeMaster14:{
        zh:"交易高手被其他用户复制跟随，会获得复制者20%的盈利分成；",
        en:"The master is copied by other users, will get 20% of the profits of the replicator;",
        vi:"Bậc Thầy được sao chép bởi những người dùng khác, sẽ nhận được 20% lợi nhuận của lệnh sao chép;",
        "zh-Hant":"交易高手被其他用戶復制跟隨，會獲得復制者20%的盈利分成；",
        id:"Master diikuti oleh pengguna lain, akan mendapatkan bagi hasil 20% dari total keuntungan;"
    },
    tobeMaster15:{
        zh:"交易高手的交易策略将被老虎外汇平台推广展示；",
        en:"Master’s trading strategies will be promoted by TigerWit;",
        vi:"Các chiến lược giao dịch của các bậc thầy sẽ được TigerWit quảng cáo;",
        "zh-Hant":"交易高手的交易策略將被老虎外匯平臺推廣展示；",
        id:"Strategi trading Master akan dipromosikan oleh TigerWit;"
    },
    tobeMaster16:{
        zh:"成为高手的交易员不可再复制其他交易高手。",
        en:"The account which became a master cannot follow other masters.",
        vi:"Các tài khoản mà đã trở thành một bậc thầy không thể sao chép các bậc thầy khác.",
        "zh-Hant":"成為高手的交易員不可再復制其他交易高手。",
        id:"Akun yang telah menjadi seorang Master, tidak dapat mengikuti Master yang lain."
    },
    tobeMaster17:{
        zh:"影响排名因素最终以TigerWit官方统计数据为准。若需人工服务，请联系客服或者发邮件至：",
        en:"The factors affecting ranking are ultimately based on TigerWit official statistics. If you need any help, please contact customer service or send e-mail to: ",
        vi:"Các yếu tố ảnh hưởng đến xếp hạng cuối cùng dựa trên thống kê chính thức của TigerWit. Nếu bạn cần bất kỳ sự trợ giúp nào, vui lòng liên hệ với dịch vụ khách hàng hoặc gửi e-mail tới: ",
        "zh-Hant":"影響排名因素最終以TigerWit官方統計數據為準。若需人工服務，請聯系客服或者發郵件至：",
        id:"Peringkat Master ditentukan dari statistik resmi TigerWit. Jika anda membutuhkan bantuan, silahkan hubungi layanan pelanggan kami atau kirimkan email ke: "
    },
    tobeMaster18:{
        zh:"开通TigerWit真实账户并完成实名认证；",
        en:"Complete ID verification and open a Live account;",
        vi:"Hoàn thành xác minh ID và mở tài khoản Live;",
        "zh-Hant":"開通TigerWit真實帳護並完成實名認證；",
        id:"Selesaikan proses verifikasi ID dan buka Akun Live."
    }
};